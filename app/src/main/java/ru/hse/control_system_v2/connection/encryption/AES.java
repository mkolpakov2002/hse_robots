...
public class AES {
    // текущий круглый индекс
    private int actual;

    // количество символов (32 бита)
    private static int Nb = 4;
    private int Nk;

    // количество раундов для текущего AES
    private int Nr;
    ...
    // Вектор инициализации для используемого здесь режима CBC
    private byte[] iv;

    // необходимая матрица для AES (sBox + инвертированная и rCon)
    ...
    private void init(byte[] key, byte[] iv) {
        ...
        // Требуются только 2 состояния с 4 строками и Nb столбцами.
        state = new int[2][4][Nb];

        // Вектор хранения для расширения создания ключа.
        w = new int[Nb * (Nr + 1)];
    ...
    // 128 бит состояния представляют собой смещение XOR, примененное к ним с израсходованными 128 битами ключа.
    // s: матрица состояний, состоящая из Nb столбцов и 4 строк.
    // Раунд: раунд ключа w, который будет добавлен.
    // s: возвращает добавление ключа за раунд
    ...
    // Основные вспомогательные методы шифрования/дешифрования (для 128-битного простого/шифрованного текста в,
    // 128-битный шифр/открытый текст), созданный алгоритмом шифрования.
...
/*
//  При шифровании/расшифровании используется режим CBC (Cipher Block Chaining).
    Он работает путем деления исходного сообщения на блоки фиксированного размера и последовательного
    шифрования каждого блока с использованием алгоритма шифрования и предыдущего зашифрованного блока.
    Каждый блок перед шифрованием XOR-ится с предыдущим зашифрованным блоком.
    Для первого блока вводится дополнительный вектор инициализации (IV), который XOR-ится с первым блоком перед шифрованием.
    Таким образом, CBC обеспечивает дополнительный уровень безопасности,
    связанный с использованием предыдущего зашифрованного блока в качестве "соли" для шифрования следующего блока.
    Это усложняет попытки атакующего выявить шаблоны в шифрованных данных и повторно использовать их для расшифровки.
 */
