...
open class MultipleTypesAdapterKt(val context: Context, deviceItemTypes: List<Device>) :
    RecyclerView.Adapter<RecyclerView.ViewHolder>() {

    data class Item(val device: Device?, var isSelected: Boolean = false, var isButton: Boolean)
    ...
    fun getSelectedItems(): ArrayList<Device> {
        val selectedArrayList: ArrayList<Device> = ArrayList()
        for(item in items){
            if (item.isSelected)
